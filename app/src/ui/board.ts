...
/**
 * Концы, на которые в этом рендере лягут тени ходов: их маркеры (кружки)
 * не рисуются — тень накрывает кружок и путает (идея 0010), а оставшиеся
 * при выбранной кости кружки читаются как «сюда она не идёт». Гейт по
 * selected повторяет условие отрисовки призраков: без выбранной кости
 * теней нет и все концы с маркерами. На черновике хода маркеры остаются
 ...
 * сцены зеркальными стали бы и пипсы 2/3/6 (их раскладка не симметрична
 * к отражению). А вот кости отражение
 ...
 * доски) затем, чтобы зеркало проверялось точечным тестом: jsdom-тесты доски
 * есть (с 17.09.2026), но «центр отразили, а угол забыли» глазами ловится
 * не всегда, а чистая функция сверяется по координатам.
 */
