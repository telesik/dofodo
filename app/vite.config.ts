...
export default defineConfig({
  ...
  test: {
    ...
    coverage: {
      ...
      // 17.09.2026: полы закреплены решением автора вслед за фактом
      // 79.4 % строк / 82.6 веток / 93.7 функций (тикет 0014, зазор ~2 пп);
      // пол functions поднят 18 → 90 — после jsdom-тестов app.ts исполняется
      // по-настоящему, артефакт метрики ушёл.
      thresholds: {
        // Храповик (решение автора 20.09.2026, telesik-team#112): полы подняты
        // вслед за фактом 85,8 / 84,2 / 96,0 / 85,8 после jsdom-тестов итогов
        // партии (#113) и бейджа цели (#112). Прежние 77/77/90/80 — с 17.09.
        lines: 85,
        statements: 85,
        functions: 95,
        branches: 84, // 07.09.2026: разово снижен 88 → 80; норма 88 ещё не возвращена — поднимать тестами app.ts
      },
    },
  },
});
